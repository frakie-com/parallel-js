			...
			try {
				if (this.requiredScripts.length !== 0) {
                  ...
                  if(this.options.useDataUri){
                    // BLOB URLs, when in data-uri HTML, start with blob:null/${uuid} which throws an "ERR_UNKNOWN_URL_SCHEME blob:null" in Chrome
					var url = "data:application/javascript,"+encodeURIComponent(src);
                  }else{
